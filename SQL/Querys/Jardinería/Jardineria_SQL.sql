...
-- 1 El total de los clientes cuya ciudad empiece por la letra M
select count(nombre_cliente)
    FROM cliente
        where ciudad like 'M%';

-- 2 El nombre del cliente con el mayor valor de venta registrado.
select max(precio_venta),nombre
    from producto;

-- 3 Obtener el codigo del cliente y pedido, cuyo pedido haya sido entregado posterior a la fecha esperada
select codigo_pedido,codigo_cliente,fecha_esperada,fecha_entrega
    from pedido
        where fecha_entrega > pedido.fecha_esperada;

-- 4 El total de clientes por pais
select count(nombre_cliente),pais
    from cliente
    group by pais;

-- 5 El precio de venta mas alto, y el precio de venta mas bajo entre los productos
select max(precio_venta), min(precio_venta)
    from producto;

-- 6 Obtener la suma total de todos los pagos que tengan un valor superior a 30000
select sum(total)
    from pago
        where total > 30000; -- No hay pagos superiores a 30.000

-- 7 calcula la fecha del primer y ultimo pago realizado por cada uno de los clientes
select min(fecha_pago), max(fecha_pago),codigo_cliente
    from pago
        where pago.codigo_cliente = any (select codigo_cliente
                                from cliente)
group by codigo_cliente;

-- 8 Obtener toda la informacion de los clientes que no aparezcan en la lista de pago
select *
    from cliente
        where codigo_cliente not in (select codigo_cliente
                                        from pago);

-- 9 Listar los clientes que hayan realizado pagos el año 2008
select codigo_cliente
    from pago
        where year(fecha_pago) = 2008;

-- 10 listado de todas las formas de pago de la tabla pago, sin repetirse
select distinct forma_pago
    from pago;

-- 11 Obtener el nombre, apellidos e email de los empleados cuyo jefe tenga el codigo de jefe 7
select nombre,apellido1,apellido2,email
    from empleado
        where codigo_jefe = 7;

-- 12 El nombre del cliente con el mayor cupo de credito.
select nombre_cliente, max(limite_credito)
    from cliente;

-- 13 Obtener toda la informacion de los clientes que sean de madrid y su vendedor asignado posea el codigo 11 o 30
select *
    from cliente
